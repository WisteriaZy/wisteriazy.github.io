...
创建一个名为 `YYYY-MM-DD-TITLE.EXTENSION` 的新文件并将其放在根目录的 `_posts` 中。请注意， `EXTENSION` 必须是 `md` 和 `markdown` 之一。如果您想节省创建文件的时间，请考虑使用插件 [`Jekyll-Compose`](https://github.com/jekyll/jekyll-compose) 来完成此操作。
...
为了准确记录帖子的发布日期，您不仅应该设置 `_config.yml` 的 `timezone` ，而且还应该在帖子头信息块的变量中提供时区。格式： `+/-TTTT` ， 例如 `+0800`。
...
> 从 `_data/authors.yml` 文件中读取作者信息的好处是页面将具有 `twitter:creator` 元标记，这丰富了 [Twitter Cards](https://developer.twitter.com/en/docs/twitter-for-websites/cards/guides/getting-started#card-and-content-attribution) ，并且有利于 SEO 。

## 目录[](#目录)

默认情况下，目录 （TOC） 显示在帖子的右侧面板上。如果要全局关闭它，请在 `_config.yml` 文件中将 `toc` 变量的值设置为 `false` 。如果要关闭特定帖子的目录，请将以下内容添加到帖子的 [头信息](https://jekyllrb.com/docs/front-matter/) 内容中：
...
评论的全局切换由 `_config.yml` 文件中的 `comments.active` 变量定义。为此变量选择评论系统后，系统将为所有帖子启用评论。
...
```markdown
```shell echo 'No more line numbers!'```
...
```markdown
{% raw %}
```liquid {%  if  product.title  contains  'Pack'  %}
...
[https://www.**twitch**.tv/videos/**1634779211**](https://www.twitch.tv/videos/1634779211)
...
[博客](/chirpy-demo-zhCN/categories/%E5%8D%9A%E5%AE%A2/), [教程](/chirpy-demo-zhCN/categories/%E6%95%99%E7%A8%8B/)

[写作](/chirpy-demo-zhCN/tags/%E5%86%99%E4%BD%9C/)
